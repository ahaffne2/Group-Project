...
```{r}
model = glm(churn ~ total_day_charge + number_customer_service_calls + total_day_minutes
...
Here we are seeing that the P-values are similar meaning that the variables together are important. 
```{r}
summary(model)
```
...
```
Checking the efficiency of our confusion matrix
```{r}
efficieny = sum(diag(table1))/sum(table1)
efficieny
```
Predicting values back on the orginal dataset
```{r}
